...
function PetView({ call, onToast }) {
  ...
  function showBubble(text) {
    ...
    setBubble(text)
    setTimeout(function () { if (tok === bubbleToken) setBubble(null) }, 6000)
  }
  function save() { try { call('save', { state: petData }).catch(function () {}) } catch (err) {} }
  function setActionTmp(a) { setAction(a); setTimeout(function () { setAction('idle') }, 2600) }
  function speakLine(text) {
    try {
      call('speak', { text: text }).then(function (r) {
  ...
  useEffect(function () {
    ...
    ;(async function () {
      let initial = null
      try { const r = await call('load'); if (r && r.state) initial = r.state } catch (err) {}
      if (!alive) return
    ...
    return function () { alive = false; clearInterval(stopPoll) }
  }, [])
  ...
  async function doGenerate() {
    ...
    try {
      const r = await call('gen-image', { prompt: prompt, image: image })
      if (r && r.url) {
  ...
  async function doGenVideo() {
    ...
    try {
      const r = await call('gen-video', { text: text, image: image })
      if (r && r.video) {
...
function apply(ctx) {
  ...
  slots.inject('shell.overlay', function () {
    return slots.register(
      ...
      function () { return h(PetView, { call: call }) },
    )
  })
}
