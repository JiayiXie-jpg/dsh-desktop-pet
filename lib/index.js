// DSH 桌宠陪伴 · Host 半（静态包入口）
//
// 依赖：inject ['timer', 'connection']；ctx.get('shell') 用于出网调用 curl / node。
// 凭据（环境变量）：
...
function fail(message) { return { ok: false, error: { code: 'bad-request', message, details: { issues: [{ message }] } } } }
...
async function genImage(shell, args) {
...
async function genVideo(shell, args) {
...
async function speak(shell, args) {
...
function apply(ctx) {
  ...
  ctx.on('tools/result', function (exec) {
    let tname = 'tool'
    try { tname = (exec && exec.name) || 'tool' } catch (err) {}
    push('tool', tname)
  })

  if (!ctx?.connection?.rpc?.handle) return

  const disposeRpc = ctx.connection.rpc.handle(RPC_CHANNEL, async (endpoint, payload = {}, signal) => {
    if (signal && signal.aborted) return fail('请求已取消')
    try {
      const shell = ctx.get('shell')
      if (endpoint === 'load') return ok({ state: state, seq: seq })
      if (endpoint === 'save') {
        if (payload && typeof payload.state === 'object' && payload.state !== null) state = payload.state
        return ok(true)
      }
      if (endpoint === 'activity') {
        const after = payload && typeof payload.after === 'number' ? payload.after : 0
        const fresh = []
        for (let i = 0; i < entries.length; i += 1) if (entries[i].seq > after) fresh.push(entries[i])
        return ok({ entries: fresh, seq: seq })
      }
      if (endpoint === 'gen-image') return ok(await genImage(shell, payload))
      if (endpoint === 'gen-video') return ok(await genVideo(shell, payload))
      if (endpoint === 'speak') return ok(await speak(shell, payload))
  ...
  ctx.effect(() => () => disposeRpc(), 'dsh-desktop-pet: dispose rpc')
}
